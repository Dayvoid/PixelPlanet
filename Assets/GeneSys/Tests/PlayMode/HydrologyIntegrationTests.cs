...
namespace GeneSys.Tests
{
    public sealed class HydrologyIntegrationTests
    {
        ...
        [UnityTest]
        public IEnumerator CapillaryEvaporationDrawsBuriedGroundwaterIntoAir()
        {
            SceneManager.LoadScene("Terrarium");
            yield return WaitForHost();
            SimulationHost host = UnityEngine.Object.FindFirstObjectByType<SimulationHost>();
            ...
            {
                Paint(host, x + dx, y - 2, MaterialIds.Mantle);
                Paint(host, x + dx, aquiferY, MaterialIds.Soil);
                Paint(host, x + dx, y, MaterialIds.Soil);
                Paint(host, x + dx, airY, MaterialIds.Air);
            }
            PaintField(host, x, y, 2f, -100f);
            PaintField(host, x, y, 5f, -100f);
            PaintField(host, x, aquiferY, 5f, -100f);
            PaintField(host, x, aquiferY, 5f, 0.5f);
            PaintField(host, x, airY, 6f, -100f);
            PaintField(host, x, y, 1f, 40f);
            yield return Step(host, 1);

            float aquiferBefore = 0f;
            float airVaporBefore = 0f;
            float trackedBefore = 0f;
            yield return ReadGpuFields(host, (mats, states, aux) =>
            {
                Assert.That(mats[y * width + x], Is.EqualTo(MaterialIds.Soil));
                Assert.That(mats[airY * width + x], Is.EqualTo(MaterialIds.Air));
                aquiferBefore = aux[aquiferY * width + x].y;
                airVaporBefore = aux[airY * width + x].x;
                trackedBefore = states[y * width + x].z + aux[y * width + x].x + aux[y * width + x].y
                    + aux[aquiferY * width + x].y + aux[airY * width + x].x + states[airY * width + x].z;
            });
            Assert.That(aquiferBefore, Is.GreaterThan(0.3f));
            ...
            yield return ReadGpuFields(host, (_, states, aux) =>
            {
                float aquiferAfter = aux[aquiferY * width + x].y;
                float airVaporAfter = aux[airY * width + x].x;
                float soilVaporAfter = aux[y * width + x].x;
                float trackedAfter = states[y * width + x].z + aux[y * width + x].x + aux[y * width + x].y
                    + aux[aquiferY * width + x].y + aux[airY * width + x].x + states[airY * width + x].z;
                Assert.That(aquiferAfter, Is.LessThan(aquiferBefore - 0.1f));
                Assert.That(airVaporAfter, Is.GreaterThan(airVaporBefore + 0.08f));
                Assert.That(soilVaporAfter, Is.LessThan(0.05f), "Humidity must leave the solid and enter Air.");
                Assert.That(trackedAfter, Is.EqualTo(trackedBefore).Within(0.08f));
            });
        }
